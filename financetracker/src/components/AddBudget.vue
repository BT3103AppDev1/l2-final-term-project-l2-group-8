...
// Assuming `db` is your Firestore instance and `collectionRef` is already defined
...
async addBudget() {
    ...
    const db = getFirestore(firebaseApp);
    const userEmail = this.user.email;
    const yearMonth = new Date().toISOString().slice(0, 7); // Current year and month
    ...
    try {
        if (this.selectedCategory === 'new') {
            ...
            // Check if the category already exists in "hello" collection
            ...
            if (!docSnap.exists()) {
                ...
                const budgetData = {
                    ...
                    budget: true,
                    expense: false,
                    expense_title: ""  // Assuming you want the full timestamp
                };
